...
function toEntropy(words) {
  ...
  var expectedChecksum = parseInt(
    CryptoJS.SHA256(CryptoJS.enc.Hex.parse(hexEntropy)).toString()[0],
    16
  ).toString(2);
...
function newPhrase() {
  ...
  let hexArr = Array.from(array)
    .map((x) => ("0" + x.toString(16)).slice(-2))
    .join("");
  var checksum = parseInt(
    CryptoJS.SHA256(CryptoJS.enc.Hex.parse(hexArr)).toString()[0],
    16
  ).toString(2);
  checksum = "0".repeat(4 - checksum.length) + checksum;
  let binary =
    Array.from(array)
      .map((x) => ("0000000" + x.toString(2)).slice(-8))
      .join("") + checksum;
  var words = [];
  for (i = 0; i < 12; i++) {
